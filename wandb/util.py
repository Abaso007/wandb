...
def get_module(
    ...
    if name not in _not_importable:
        try:
            return import_module(name) if not lazy else import_module_lazy(name)
        except Exception:
...
pd_available = pandas_spec is not None
...
def download_file_from_url(
    ...
    auth = None if _thread_local_api_settings.cookies else ("api", api_key or "")
